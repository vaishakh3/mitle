...
Recorded through 2026-08-24 in Asia/Kolkata. This is an evidence log, not a substitute for the human gates in `RELEASE_READINESS.md`.
...
- Root suite: 75/75 tests passed; isolated Lambda suite: 25/25 tests passed.
...
- Final `npm run release:verify` passed end to end on 2026-08-24, including web export, SAM validation/build, strict TypeScript, Expo dependency alignment, and Expo Doctor 21/21.
...
The endpoint-bound version-6 clean build completed 2026-08-24 and produced the signed APK and Play AAB from one generated native tree.

- Package `app.milte`; version `1.0.0` (6); min SDK 24; target SDK 36.
...
- APK: 54,296,872 bytes; SHA-256 `3d664dffdae5678304af948ce609f7069808e994aff7d1905acffbb9cfac8fba`.
- AAB: 41,519,754 bytes; SHA-256 `79647c68c27f9b914557f2db27bbfe40d6ea247fd1cbaee99c1247f95fa21b0b`.
...
- The API 24 emulator fresh-installed the exact version 6 APK, resolved `app.milte/.MainActivity`, cold-launched the branded sign-in screen, confirmed version/min/target metadata, and produced no fatal/React Native/Metro/localhost error. A real production existing-user canary then received the eight-digit Cognito code `24635871`; the exact APK displayed length-neutral copy, kept Resend and Use another email inside the short viewport, accepted all eight digits, authenticated, and reached onboarding.
- The API 36 emulator fresh-installed the exact version 6 APK, cold-launched cleanly, kept the scrolling content below the status bar, confirmed version/min/target metadata, and produced no fatal/React Native/Metro/localhost error. Version 5 remains the evidence-bearing full seven-step onboarding/location/data/Today traversal; version 6 changes only the sign-in validation/copy/layout covered by the exact-build API 24 journey and automated suites.
...
- Web invalidation `IUCKDMOAHU7U6XINC1RMH9UY9` completed after the version-6 source-frozen export. The live bundle contains the neutral OTP instruction and corrected mismatch guidance, and no longer contains the stale six-digit instruction.
...
- Release `1.0.0 (6) — OTP and layout fix` uses the frozen English notes and a 100% Alpha closed-test rollout. On 2026-08-24, Publishing overview moved it to **Changes in review** while Play's automated quick checks continued; Play will forward the change to review as soon as those checks complete successfully. Version 5 remains tester-visible until Play approves and publishes version 6.
